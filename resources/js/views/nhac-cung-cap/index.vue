    ...
    <el-col :span="24">
      <el-card>
        <div slot="header" class="tw-flex tw-justify-end tw-items-center">
          <router-link v-slot="{ href, navigate }" v-permission="['create']" :to="{name: 'NhacCungCapCreate'}" custom>
        ...
        <el-row :gutter="20">
          <el-col :span="24" class="table-responsive">
            ...
            >
              <el-table-column align="center" sortable="custom" prop="id" :label="$t('table.nhac_cung_cap.id')" width="70px">
                <template slot-scope="{ row }">
                  {{ row.id }}
                </template>
              </el-table-column>
              <el-table-column data-generator="man_nha_cung_cap" prop="man_nha_cung_cap" :label="$t('table.nhac_cung_cap.man_nha_cung_cap')" align="left" header-align="center">
                <template slot-scope="{ row }">
                  {{ row.man_nha_cung_cap }}
                </template>
              </el-table-column>
              <el-table-column data-generator="ten_nhac_cung_cap" prop="ten_nhac_cung_cap" :label="$t('table.nhac_cung_cap.ten_nhac_cung_cap')" align="left" header-align="center">
                <template slot-scope="{ row }">
                  {{ row.ten_nhac_cung_cap }}
                </template>
              </el-table-column>
              <el-table-column data-generator="dia_chi" prop="dia_chi" :label="$t('table.nhac_cung_cap.dia_chi')" align="left" header-align="center">
                <template slot-scope="{ row }">
                  {{ row.dia_chi }}
                </template>
              </el-table-column>
              <el-table-column data-generator="so_dien_thoai" prop="so_dien_thoai" :label="$t('table.nhac_cung_cap.so_dien_thoai')" align="left" header-align="center">
                <template slot-scope="{ row }">
                  {{ row.so_dien_thoai }}
                </template>
              </el-table-column>
              <el-table-column data-generator="tk_ngan_hang" prop="tk_ngan_hang" :label="$t('table.nhac_cung_cap.tk_ngan_hang')" align="left" header-align="center">
                <template slot-scope="{ row }">
                  {{ row.tk_ngan_hang }}
                </template>
              </el-table-column>
              ...
              <el-table-column :label="$t('table.actions')" align="center" class-name="small-padding fixed-width">
                <template slot-scope="{ row }">
                  <router-link v-permission="['edit']" :title="$t('button.edit')" :to="{name: 'NhacCungCapEdit', params: {id: row.id}}"><i class="el-icon-edit el-link el-link--primary tw-mr-2" /></router-link>
...
<script>
  ...
  import NhacCungCapsResource from '@/api/v1/nhac-cung-cap';
  const nhacCungCapResource = new NhacCungCapsResource();
    ...
    methods: {
      async getList() {
        try {
          ...
          const { data } = await nhacCungCapResource.list(this.table.listQuery);
      ...
      remove(id) {
        this.$confirm(this.$t('messages.delete_confirm', { attribute: this.$t('table.nhac_cung_cap.id') + '#' + id }), this.$t('messages.warning'), {
          ...
          try {
            this.table.loading = true;
            await nhacCungCapResource.destroy(id);
