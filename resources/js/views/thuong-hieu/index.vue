    ...
    <el-col :span="24">
      <el-card>
        <div slot="header" class="tw-flex tw-justify-end tw-items-center">
          <router-link v-slot="{ href, navigate }" v-permission="['create']" :to="{name: 'ThuongHieuCreate'}" custom>
        ...
        <el-row :gutter="20">
          <el-col :span="24" class="table-responsive">
            ...
            >
              <el-table-column align="center" sortable="custom" prop="id" :label="$t('table.thuong_hieu.id')" width="70px">
                <template slot-scope="{ row }">
                  {{ row.id }}
                </template>
              </el-table-column>
              <el-table-column data-generator="ten_hang" prop="ten_hang" :label="$t('table.thuong_hieu.ten_hang')" align="left" header-align="center">
                <template slot-scope="{ row }">
                  {{ row.ten_hang }}
                </template>
              </el-table-column>
              <el-table-column data-generator="mo_ta" prop="mo_ta" :label="$t('table.thuong_hieu.mo_ta')" align="left" header-align="center">
                <template slot-scope="{ row }">
                  {{ row.mo_ta }}
                </template>
              </el-table-column>
              ...
              <el-table-column :label="$t('table.actions')" align="center" class-name="small-padding fixed-width">
                <template slot-scope="{ row }">
                  <router-link v-permission="['edit']" :title="$t('button.edit')" :to="{name: 'ThuongHieuEdit', params: {id: row.id}}"><i class="el-icon-edit el-link el-link--primary tw-mr-2" /></router-link>
...
<script>
  ...
  import ThuongHieusResource from '@/api/v1/thuong-hieu';
  const thuongHieuResource = new ThuongHieusResource();
    ...
    methods: {
      async getList() {
        try {
          ...
          const { data } = await thuongHieuResource.list(this.table.listQuery);
      ...
      remove(id) {
        this.$confirm(this.$t('messages.delete_confirm', { attribute: this.$t('table.thuong_hieu.id') + '#' + id }), this.$t('messages.warning'), {
          ...
          try {
            this.table.loading = true;
            await thuongHieuResource.destroy(id);
