    ...
    <el-col :span="24">
      <el-card>
        <div slot="header" class="tw-flex tw-justify-end tw-items-center">
          <router-link v-slot="{ href, navigate }" v-permission="['create']" :to="{name: 'DanhMucCreate'}" custom>
        ...
        <el-row :gutter="20">
          <el-col :span="24" class="table-responsive">
            ...
            >
              <el-table-column align="center" sortable="custom" prop="id" :label="$t('table.danh_muc.id')" width="70px">
                <template slot-scope="{ row }">
                  {{ row.id }}
                </template>
              </el-table-column>
              <el-table-column data-generator="ten_danh_muc" prop="ten_danh_muc" :label="$t('table.danh_muc.ten_danh_muc')" align="left" header-align="center">
                <template slot-scope="{ row }">
                  {{ row.ten_danh_muc }}
                </template>
              </el-table-column>
              <el-table-column data-generator="mo_ta" prop="mo_ta" :label="$t('table.danh_muc.mo_ta')" align="left" header-align="center">
                <template slot-scope="{ row }">
                  {{ row.mo_ta }}
                </template>
              </el-table-column>
              ...
              <el-table-column :label="$t('table.actions')" align="center" class-name="small-padding fixed-width">
                <template slot-scope="{ row }">
                  <router-link v-permission="['edit']" :title="$t('button.edit')" :to="{name: 'DanhMucEdit', params: {id: row.id}}"><i class="el-icon-edit el-link el-link--primary tw-mr-2" /></router-link>
...
<script>
  ...
  import DanhMucsResource from '@/api/v1/danh-muc';
  const danhMucResource = new DanhMucsResource();
    ...
    methods: {
      async getList() {
        try {
          ...
          const { data } = await danhMucResource.list(this.table.listQuery);
      ...
      remove(id) {
        this.$confirm(this.$t('messages.delete_confirm', { attribute: this.$t('table.danh_muc.id') + '#' + id }), this.$t('messages.warning'), {
          ...
          try {
            this.table.loading = true;
            await danhMucResource.destroy(id);
