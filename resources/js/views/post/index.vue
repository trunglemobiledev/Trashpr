    ...
    <el-col :span="24">
      <el-card>
        <div slot="header" class="tw-flex tw-justify-end tw-items-center">
          <router-link v-slot="{ href, navigate }" v-permission="['create']" :to="{name: 'PostCreate'}" custom>
        ...
        <el-row :gutter="20">
          <el-col :span="24" class="table-responsive">
            ...
            >
              <el-table-column align="center" sortable="custom" prop="id" :label="$t('table.post.id')" width="70px">
                <template slot-scope="{ row }">
                  {{ row.id }}
                </template>
              </el-table-column>
              <el-table-column data-generator="name" prop="name" :label="$t('table.post.name')" align="left" header-align="center">
                <template slot-scope="{ row }">
                  {{ row.name }}
                </template>
              </el-table-column>
              <el-table-column data-generator="comment.id" prop="comment.id" :label="$t('route.comment')" align="left" header-align="center">
                  <template slot-scope="{ row }">
                    <template v-for="(item) in row.comments">
                      <el-tag
                        :key="'comment.id_' + item.id"
                        class="tw-mr-2"
                        size="medium"
                      >
                        {{ item.id }}
            ...
            <!--{{$TEMPLATES_NOT_DELETE_THIS_LINE$}}-->
              ...
              <el-table-column :label="$t('table.actions')" align="center" class-name="small-padding fixed-width">
                <template slot-scope="{ row }">
                  <router-link v-permission="['edit']" :title="$t('button.edit')" :to="{name: 'PostEdit', params: {id: row.id}}"><i class="el-icon-edit el-link el-link--primary tw-mr-2" /></router-link>
...
<script>
  ...
  import PostsResource from '@/api/v1/post';
  const postResource = new PostsResource();
    ...
    methods: {
      async getList() {
        try {
          ...
          const { data } = await postResource.list(this.table.listQuery);
      ...
      remove(id) {
        this.$confirm(this.$t('messages.delete_confirm', { attribute: this.$t('table.post.id') + '#' + id }), this.$t('messages.warning'), {
