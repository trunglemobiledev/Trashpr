    ...
    <el-col :span="24">
      <el-card>
        <div slot="header" class="tw-flex tw-justify-end tw-items-center">
          <router-link v-slot="{ href, navigate }" v-permission="['create']" :to="{name: 'KhoCreate'}" custom>
        ...
        <el-row :gutter="20">
          <el-col :span="24" class="table-responsive">
            ...
            >
              <el-table-column align="center" sortable="custom" prop="id" :label="$t('table.kho.id')" width="70px">
                <template slot-scope="{ row }">
                  {{ row.id }}
                </template>
              </el-table-column>
              <el-table-column data-generator="ten_kho" prop="ten_kho" :label="$t('table.kho.ten_kho')" align="left" header-align="center">
                <template slot-scope="{ row }">
                  {{ row.ten_kho }}
                </template>
              </el-table-column>
              <el-table-column data-generator="dia_chi" prop="dia_chi" :label="$t('table.kho.dia_chi')" align="left" header-align="center">
                <template slot-scope="{ row }">
                  {{ row.dia_chi }}
                </template>
              </el-table-column>
              <el-table-column data-generator="mo_ta" prop="mo_ta" :label="$t('table.kho.mo_ta')" align="left" header-align="center">
                <template slot-scope="{ row }">
                  {{ row.mo_ta }}
                </template>
              </el-table-column>
              <el-table-column data-generator="ma_kho" prop="ma_kho" :label="$t('table.kho.ma_kho')" align="left" header-align="center">
                <template slot-scope="{ row }">
                  {{ row.ma_kho }}
                </template>
              </el-table-column>
              ...
              <el-table-column :label="$t('table.actions')" align="center" class-name="small-padding fixed-width">
                <template slot-scope="{ row }">
                  <router-link v-permission="['edit']" :title="$t('button.edit')" :to="{name: 'KhoEdit', params: {id: row.id}}"><i class="el-icon-edit el-link el-link--primary tw-mr-2" /></router-link>
...
<script>
  ...
  import KhosResource from '@/api/v1/kho';
  const khoResource = new KhosResource();
    ...
    methods: {
      async getList() {
        try {
          ...
          const { data } = await khoResource.list(this.table.listQuery);
      ...
      remove(id) {
        this.$confirm(this.$t('messages.delete_confirm', { attribute: this.$t('table.kho.id') + '#' + id }), this.$t('messages.warning'), {
