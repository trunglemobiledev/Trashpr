    ...
    <el-col :span="24">
      <el-card>
        <div slot="header" class="tw-flex tw-justify-end tw-items-center">
          <router-link v-slot="{ href, navigate }" v-permission="['create']" :to="{name: 'XuatKhoCreate'}" custom>
        ...
        <el-row :gutter="20">
          <el-col :span="24" class="table-responsive">
            ...
            >
              <el-table-column align="center" sortable="custom" prop="id" :label="$t('table.xuat_kho.id')" width="70px">
                <template slot-scope="{ row }">
                  {{ row.id }}
                </template>
              </el-table-column>
              <el-table-column data-generator="ma_phieu_xuat" prop="ma_phieu_xuat" :label="$t('table.xuat_kho.ma_phieu_xuat')" align="left" header-align="center">
                <template slot-scope="{ row }">
                  {{ row.ma_phieu_xuat }}
                </template>
              </el-table-column>
              <el-table-column data-generator="ngay_xuat" prop="ngay_xuat" :label="$t('table.xuat_kho.ngay_xuat')" align="center" header-align="center">
                <template slot-scope="{ row }">
                  {{ row.ngay_xuat }}
                </template>
              </el-table-column>
              <el-table-column data-generator="so_luong" prop="so_luong" :label="$t('table.xuat_kho.so_luong')" align="left" header-align="center">
                <template slot-scope="{ row }">
                  {{ row.so_luong }}
                </template>
              </el-table-column>
            ...
            <!--{{$TEMPLATES_NOT_DELETE_THIS_LINE$}}-->
              ...
              <el-table-column :label="$t('table.actions')" align="center" class-name="small-padding fixed-width">
                <template slot-scope="{ row }">
                  <router-link v-permission="['edit']" :title="$t('button.edit')" :to="{name: 'XuatKhoEdit', params: {id: row.id}}"><i class="el-icon-edit el-link el-link--primary tw-mr-2" /></router-link>
...
<script>
  ...
  import XuatKhosResource from '@/api/v1/xuat-kho';
  const xuatKhoResource = new XuatKhosResource();
    ...
    methods: {
      async getList() {
        try {
          ...
          const { data } = await xuatKhoResource.list(this.table.listQuery);
      ...
      remove(id) {
        this.$confirm(this.$t('messages.delete_confirm', { attribute: this.$t('table.xuat_kho.id') + '#' + id }), this.$t('messages.warning'), {
          ...
          try {
            this.table.loading = true;
            await xuatKhoResource.destroy(id);
